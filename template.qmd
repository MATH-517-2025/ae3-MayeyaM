...
NB: To use the code, please read the section "[**How to simply plot results**]{.underline}". Each function is callable from the same `.py` file and terminal.
...
![Visual plot of bandwith impact, call of `py plot_results.py simple_plot 10 10 10000 0.07 1`](simple_plot_bandwith_10_10.png)

*NB:* *Note that to produce these variations, one must directly change export name of the `.png` files.*

-   The choice of $N$ should possibly depend on $\alpha,\beta$ (and $m$) as the concentrations of samples will vary in $[0,1]$. Given $m$ and depending on $N$, we may not be able to capture the volatility of $\theta_{22}$ accurately.
...
To complete the observations around the impacts of $\alpha,\beta$ on the estimations, we perform a last run for $\alpha,\beta$ taking values in `np.arange(0.01, 1, step = 0.01)`. There, we observe a fade from the left and upper boundaries to the center for $\theta_{22}$, and a strong boundary effect on $h_{IMSE}$, as already expected. We additionally observe that the estimations of $h_{IMSE}$ remain stable for values of $\alpha,\beta$ far enough from zero (both at the same time) and that the estimations of $\sigma^2$ are reasonably stable on average, though very "white-noise" looking.
...
-   initial estimations are that $h_{IMSE}$ is below $0.2$ on average, $\sigma^2$ is, as expected, around $1.0$ and $\theta_{22}$ appears to be around $50000$ on average.
...
![Impact of \$n\$ and \$N\$ when \$\\alpha,\\beta=10\$](sample_size_block_number_impact_colormap_10_10.png)

We observe that

-   Similarly to before, as $n$ increases, the estimations become more stable in comparison to the average.

-   On the other hand, the choice of $N$ appears to impact the stability significantly (especially for $\theta_{22}$ and $h_{IMSE}$) in comparison to the previous case where $(\alpha,\beta)=(0.1, 0.2)$. $N$ will change significantly how the estimates are captured when the density of samples move (this is controlled by $\alpha,\beta$. We can deduce that $N$ should (or at least could) depend on $\alpha,\beta$.
...
We start by fixing $(\alpha,\beta)=(0.1, 0.2)$ to be in the situation we have already extensively discussed.

We observe, as expected and previously observed, that small sample sized (even with optimal number of blocks) yield unreasonable estimations in comparison to the average. Moreover, the estimation of $h_{IMSE}$ appears to average around the value of $0.07$ after the elbow (see second figure for clear elbow). The estimation of $\sigma^2$ also becomes stable and converges to, it appears close to, $1.0$ (as expected). Furthermore, as already observed previously, the estimation of $\theta_{22}$ is relatively unstable / changes a lot.
...
![Mallow selected evolution up to 2000 samples](Mallow_evolution_2000.png)

Here below is the call of `py plot_results.py simple_plot 0.1 0.2 10000 0.07 1` with the bandwith of $0.7$ mentioned above.

![Plot with bandwith of 0.7](simple_plot_bandwith_2.png)

Again, changing $\alpha,\beta$ produces significantly different results, as shown here below when calling `py plot_results.py sample_size_mallow_evolution 20 10 1000 10 1.0 0.1`, that is we let $(\alpha,\beta)=(20, 10)$:

![Call of `py plot_results.py sample_size_mallow_evolution 20 10  1000 10 1.0 0.1`](plot_sample_size_mallow_20_10.png)

This time on the other hand, the computation of $\theta_{22}$ becomes stable, the estimation of $\sigma^2$ is still relatively stable, closely around $1.0$. and $h_{IMSE}$ is estimated around $0.09$ after the elbow. $\theta_{22}$ is close to $10000$ after the computations stabilize.

-   This again confirms the suspicion that $\alpha$, $\beta$ significantly impact $h_{IMSE}$ and the other estimations, especially $\theta_{22}$.
